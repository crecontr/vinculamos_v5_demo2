...
<div class="wrapper">
	<?php
		...
		include_once("modal_add_campus.php");
		include_once("modal_delete_campus.php");
	?>
    						...
    						<?php
    							if(canCreateParameters()) {?>
    								<div class="btn-group pull-right">
										<button id="exportButton" name="exportButton" class="btn btn-orange pull-right" data-toggle="modal" data-target="#addCampus">
											<span class="fa fa-plus"></span> Agregar Sede
										</button>
									</div>
							<?php
								} ?>
...
<script>
	...
	$("#add_campus").submit(function( event ) {
		$('#add_campus').attr("disabled", true);
		var parametros = $(this).serialize();
		$.ajax({
			type: "POST",
			url: "./ajax_add_campus.php",
			data: parametros,
			beforeSend: function(objeto) {
				$("#resultados_modal_agregar").html("Cargando...");
			},
			success: function(datos) {
				$('#add_campus').attr("disabled", false);
	...
	$('#addCampus').on('show.bs.modal', function (event) {
	...
	$("#delete_campus").submit(function( event ) {
		$('#delete_campus').attr("disabled", true);
		var parametros = $(this).serialize();
		$.ajax({
			type: "POST",
			url: "./ajax_delete_campus.php",
			data: parametros,
			beforeSend: function(objeto) {
				$("#mensaje_resultados").html("Cargando...");
			},
			success: function(datos) {
				$('#delete_campus').attr("disabled", false);
	...
	$('#deleteCampus').on('show.bs.modal', function (event) {
		var button = $(event.relatedTarget); // Button that triggered the modal
