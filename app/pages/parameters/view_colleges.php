...
<div class="wrapper">
	<?php
		...
		include_once("modal_add_college.php");
		include_once("modal_delete_college.php");
	?>
	...
	<div class="content-wrapper">
    	...
    	<section class="content-header">
      		<h1>
        		...
        		<small>todas las unidades institucionales</small>
      		</h1>
      		<ol class="breadcrumb">
        		<li><i class="fa fa-home"></i> Inicio</li>
        		<li>Unidad Institucional</li>
        		<li class="active">Ver Unidad</li>
      		</ol>
    	</section>
				...
				<div class="col-xs-12">
					<div class="box">
						<div class="box-header">
    						<h3 class="box-title">Listado de unidades institucionales</h3>

    						<?php
    							if(canCreateParameters()) {?>
    								<div class="btn-group pull-right">
										<button id="exportButton" name="exportButton" class="btn btn-orange pull-right" data-toggle="modal" data-target="#addCollege">
											<span class="fa fa-plus"></span> Agregar Unidad Institucional
										</button>
									</div>
							<?php
								} ?>
...
<script>
	...
	$("#add_college").submit(function( event ) {
		$('#add_college').attr("disabled", true);
		var parametros = $(this).serialize();
		$.ajax({
			type: "POST",
			url: "./ajax_add_college.php",
			data: parametros,
			beforeSend: function(objeto) {
				$("#resultados_modal_agregar").html("Cargando...");
			},
			success: function(datos) {
				$('#add_college').attr("disabled", false);
	...
	$('#addCollege').on('show.bs.modal', function (event) {
	...
	$("#delete_college").submit(function( event ) {
		$('#delete_college').attr("disabled", true);
		var parametros = $(this).serialize();
		$.ajax({
			type: "POST",
			url: "./ajax_delete_college.php",
			data: parametros,
			beforeSend: function(objeto) {
				$("#mensaje_resultados").html("Cargando...");
			},
			success: function(datos) {
				$('#delete_college').attr("disabled", false);
