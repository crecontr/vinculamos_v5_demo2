<?php
	...
	$datas = getVisibleCollegesByInstitution($institucion);
?>
	  				...
	  				<?php
	  					if(canUpdateParameters()) {
	  						$data = noeliaEncode("data" . $datas[$i]['id']); ?>
								<a href="edit_college.php?data=<?php echo$data;?>" class='btn btn-orange' title='Editar'>
						...
						<?php
	  					if(canDeleteParameters()) { ?>
								<a href="#" class='btn btn-default' title='Eliminar'
									...
									data-toggle="modal" data-target="#deleteCollege">
